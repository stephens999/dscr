...
```{r}
add_scenario(dsc_osl,name="normal",datamaker,args=list(disttype="normal",nsamp=1000),seed=1:100)
...
Finally, dscr has the ability to add a method that can ``cheat" by taking not only the input data, but also the meta data. Sometimes this can be useful
...
For instance a gold standard method might perform (in some sense optimal) inference under the model used to simulate the data...
```{r}
  #define a wrapper function that takes both input and meta for its arguments
